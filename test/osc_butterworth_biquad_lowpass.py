...
class OscServer(ServerThread):
    ...
    @make_method('/cyperus/add/module/butterworth_biquad_lowpass', 'sff')
    def osc_add_module_butterworth_biquad_lowpass(self, path, args):
        print("received '/cyperus/add/module/butterworth_biquad_lowpass'")
        responses.put(args)

    @make_method('/cyperus/edit/module/butterworth_biquad_lowpass', 'sff')
    def osc_edit_module_butterworth_biquad_lowpass(self, path, args):
        print("received '/cyperus/edit/module/butterworth_biquad_lowpass'")
        responses.put(args)
...
def test_single_channel_single_bus_sine_follower_butterworth_biquad_lowpass(dest):
    mains = {'in': [],
             'out': []}
    bus_main0_uuid = None
    bus_ports  = {'in': [],
                  'out': []}
    butterworth_biquad_lowpass_module_uuid = None
    butterworth_biquad_lowpass_module_ports = {'in': [],
                          'out': []}
    ...
    liblo.send(dest, "/cyperus/add/module/butterworth_biquad_lowpass", "/{}".format(bus_main0_uuid), 150.0, 0.05)
    response = responses.get()
    butterworth_biquad_lowpass_module_uuid = response[0]    
    
    liblo.send(dest, "/cyperus/list/module_port", "/{}?{}".format(bus_main0_uuid,
                                                                  butterworth_biquad_lowpass_module_uuid))
    response = responses.get()
    print('bloc_processor response: {}'.format(response))
    raw_butterworth_biquad_lowpass_module_ports = response[1].split('\n')
    print(raw_butterworth_biquad_lowpass_module_ports)
    outs = False
    for elem in filter(None, raw_butterworth_biquad_lowpass_module_ports):
        if 'out:' in elem:
            outs = True
        elif 'in:' in elem:
            pass
        elif outs:
            butterworth_biquad_lowpass_module_ports['out'].append(elem)
        else:
            butterworth_biquad_lowpass_module_ports['in'].append(elem)
    print('butterworth_biquad_lowpass_module_ports', butterworth_biquad_lowpass_module_ports)
    ...
    liblo.send(dest,
               ...
               "/{}?{}<{}".format(bus_main0_uuid,
                                  butterworth_biquad_lowpass_module_uuid,
                                  butterworth_biquad_lowpass_module_ports['in'][0].split('|')[0]))
    response = responses.get()
    ...
    liblo.send(dest,
               ...
               "/{}?{}>{}".format(bus_main0_uuid,
                                  butterworth_biquad_lowpass_module_uuid,
                                  butterworth_biquad_lowpass_module_ports['out'][0].split('|')[0]),
               "/{}:{}".format(bus_main0_uuid,
    ...
    liblo.send(dest, "/cyperus/edit/module/butterworth_biquad_lowpass", "/{}?{}".format(bus_main0_uuid, butterworth_biquad_lowpass_module_uuid), 75.0, 0.05)
    response = responses.get()

    time.sleep(5)
    
    liblo.send(dest, "/cyperus/edit/module/butterworth_biquad_lowpass", "/{}?{}".format(bus_main0_uuid, butterworth_biquad_lowpass_module_uuid), 300.0, 0.05)
    response = responses.get()
...
if __name__ == '__main__':
    ...
    test_single_channel_single_bus_sine_follower_butterworth_biquad_lowpass(dest)
