...
class OscServer(ServerThread):
    ...
    @make_method('/cyperus/add/module/highpass', 'sff')
    def osc_add_module_highpass(self, path, args):
        print("received '/cyperus/add/module/highpass'")
        responses.put(args)

    @make_method('/cyperus/edit/module/highpass', 'sff')
    def osc_edit_module_highpass(self, path, args):
        print("received '/cyperus/edit/module/highpass'")
        responses.put(args)
...
def test_single_channel_single_bus_sine_follower_highpass(dest):
    mains = {'in': [],
             'out': []}
    bus_main0_uuid = None
    bus_ports  = {'in': [],
                  'out': []}
    highpass_module_uuid = None
    highpass_module_ports = {'in': [],
                          'out': []}
    ...
    liblo.send(dest, "/cyperus/add/module/highpass", "/{}".format(bus_main0_uuid), 1.0, 440.0)
    response = responses.get()
    highpass_module_uuid = response[0]    
    
    liblo.send(dest, "/cyperus/list/module_port", "/{}?{}".format(bus_main0_uuid,
                                                                  highpass_module_uuid))
    response = responses.get()
    print('bloc_processor response: {}'.format(response))
    raw_highpass_module_ports = response[1].split('\n')
    print(raw_highpass_module_ports)
    outs = False
    for elem in filter(None, raw_highpass_module_ports):
        if 'out:' in elem:
            outs = True
        elif 'in:' in elem:
            pass
        elif outs:
            highpass_module_ports['out'].append(elem)
        else:
            highpass_module_ports['in'].append(elem)
    print('highpass_module_ports', highpass_module_ports)
    ...
    liblo.send(dest,
               ...
               "/{}?{}<{}".format(bus_main0_uuid,
                                  highpass_module_uuid,
                                  highpass_module_ports['in'][0].split('|')[0]))
    response = responses.get()
    ...
    liblo.send(dest,
               ...
               "/{}?{}>{}".format(bus_main0_uuid,
                                  highpass_module_uuid,
                                  highpass_module_ports['out'][0].split('|')[0]),
               "/{}:{}".format(bus_main0_uuid,
    ...
    liblo.send(dest, "/cyperus/edit/module/highpass", "/{}?{}".format(bus_main0_uuid, highpass_module_uuid), 1.0, 1200.0)
    response = responses.get()
...
if __name__ == '__main__':
    ...
    test_single_channel_single_bus_sine_follower_highpass(dest)
