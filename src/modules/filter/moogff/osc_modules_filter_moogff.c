...
#include "osc_modules_filter_moogff.h"
						   ...
						   int argc, void *data, void *user_data)
{
  ...
  lo_send(lo_addr_send,"/cyperus/add/module/filter/moogff","sisfffff", request_id, 0, module_id, frequency, gain, reset, mul, add);
  free(lo_addr_send);

  return 0;
} /* osc_add_modules_filter_moogff_handler */
...
{  
  ...
  lo_address lo_addr_send = lo_address_new((const char*)send_host_out, (const char*)send_port_out);
  lo_send(lo_addr_send,"/cyperus/edit/module/filter/moogff","sisfffff", request_id, 0, module_id, frequency, gain, reset, mul, add);
  free(lo_addr_send);
