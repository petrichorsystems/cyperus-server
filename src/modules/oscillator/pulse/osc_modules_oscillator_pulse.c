						   ...
						   int argc, void *data, void *user_data)
{
  ...
  lo_send(lo_addr_send,"/cyperus/add/module/oscillator/pulse","sisffff", request_id, 0, module_id, frequency, pulse_width, mul, add);
  free(lo_addr_send);

  return 0;
} /* osc_add_modules_oscillator_pulse_handler */


int
osc_edit_modules_oscillator_pulse_handler(const char *path, const char *types, lo_arg ** argv,
						int argc, void *data, void *user_data)
{  
  ...
  lo_address lo_addr_send = lo_address_new((const char*)send_host_out, (const char*)send_port_out);
  lo_send(lo_addr_send,"/cyperus/edit/module/oscillator/pulse","sisffff", request_id, 0, module_id, frequency, pulse_width, mul, add);
  free(lo_addr_send);
  
  return 0;
} /* osc_edit_modules_oscillator_pulse_handler */
