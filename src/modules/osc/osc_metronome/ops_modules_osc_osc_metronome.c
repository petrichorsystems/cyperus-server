...
dsp_create_osc_metronome(struct dsp_bus *target_bus,
  ...
  osc_metronome_param.parameters->int32_type = malloc(sizeof(int) * 2);
  osc_metronome_param.parameters->int32_type[0] = 0;
  osc_metronome_param.parameters->int32_type[1] = (int)((float)1.0 / (osc_metronome_param.parameters->float32_type[0] / 60.0f) * (float)jackcli_samplerate);
...
dsp_osc_metronome(struct dsp_operation *osc_metronome,
  ...
  int samples_waited = osc_metronome->module->dsp_param.parameters->int32_type[0];
  int samples_to_wait = osc_metronome->module->dsp_param.parameters->int32_type[1];
  if( samples_waited == samples_to_wait - 1) {
    ...
    free(lo_addr_send);
    outsample = 1.0f;
    osc_metronome->module->dsp_param.parameters->int32_type[0] = 0;
  } else {
    osc_metronome->module->dsp_param.parameters->int32_type[0] += 1;
  }
...
void dsp_edit_osc_metronome(struct dsp_module *osc_metronome,
  ...
  osc_metronome->dsp_param.parameters->int32_type[0] = 0;
  osc_metronome->dsp_param.parameters->int32_type[1] = (int)(1.0f / (osc_metronome->dsp_param.parameters->float32_type[0] / 60.0f) * (float)jackcli_samplerate);
  printf("returning\n");
